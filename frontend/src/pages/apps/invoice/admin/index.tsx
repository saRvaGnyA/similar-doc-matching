...
const defaultColumns = [
  {
    ...
    renderCell: ({ row }: CellType) => (
      <StyledLink href={`/apps/invoice/admin/${row.id}`}>{`${row.id.substring(
        0,
        6
      )}...`}</StyledLink>
    ),
...
const InvoiceList = () => {
  ...
  useEffect(() => {
    ...
    let Q = `query MyQuery {
      Claim {
        ...
        status
        user_id
        Documents_aggregate(where: {similarity_score: {_gte: "0.5"}}) {
          aggregate {
            count
          }
        }
  ...
  const handleStatusValue = (e: SelectChangeEvent) => {
    ...
    if (e.target.value === "") {
      let Q = `query MyQuery {
        Claim(order_by: {created_at: desc}) {
          amount
          created_at
          date
          id
      ...
      let Q = `query MyQuery {
        Claim(order_by: {created_at: desc}, where: {status: {_eq: "${e.target.value}"}}) {
          amount
          created_at
          date
          id
  ...
  return (
    <DatePickerWrapper>
      <Grid container spacing={6}>
        <Grid item xs={12}>
          <Card>
            <CardHeader title="Claims by Users" />
            <CardContent>
              <Grid container spacing={6}>
